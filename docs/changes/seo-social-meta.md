...
## MVP scope
1. **Statyczny head w `index.html`** (źródło prawdy dla crawlerów — HashRouter serwuje jeden dokument na wszystkie trasy): opisowy `<title>`, `meta description`, `robots`, `canonical`, `theme-color` (light + dark przez `prefers-color-scheme`), pełne `og:*` (type/url/title/description/image/locale/site_name), `twitter:*` (summary_large_image), JSON-LD `WebApplication`. Copy po angielsku, `lang="en"` bez zmian — tak stanowi ADR-0003 (sprostowanie pierwotnego założenia `lang="pl"` z tego planu).
