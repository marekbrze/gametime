...
/**
 * Podmienia katalog runtime na drużyny ze snapshota (`public/data.json` z
 * data-pipeline) — snapshot jest jedyną prawdą o realnym katalogu (155 drużyn),
 * scalanie ze statycznym mockowym katalogiem dublowałoby nazwy (statyczne
 * `nhl-tor` + realne `espn-nhl-*` to te same drużyny pod innymi id).
 * Tablicę i mapę mutujemy w miejscu — konsumenci (event-labels, teams,
 * calendar-export) trzymają referencje. Ścieżka mockowa (dev fallback /
 * devEvents override) nie woła tej funkcji, więc mocki trzymają statyczny katalog.
 */
export function registerCatalogTeams(teams: Team[]): void {
  TEAMS.splice(0, TEAMS.length, ...teams);
  TEAM_BY_ID.clear();
  for (const team of teams) TEAM_BY_ID.set(team.id, team);
}
